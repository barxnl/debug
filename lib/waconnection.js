...
module.exports = class WAConnection {
  constructor(client) {
    ...
    this.client = client
    this.chats = {}
    this.MessageType = Object.fromEntries(Object.keys(this.proto).filter(tr => tr.endsWith('Message')).map(tr => {
  ...
  async sendMessageFromContent(mess, obj = {}) {
    let types = Object.values(this.MessageType)
    let type = Object.keys(obj).find(tr => types.includes(tr) && !tr.includes("senderKey") && !tr.includes('context'))
    ...
    let prepare = await this.generateWAMessageFromContent(typeof mess == 'object'?mess.key.remoteJid: mess, obj, obj)
