...
module.exports = class Functions {
  ...
  async metadataMsg(client, msg) {
    let chatMeta = async(mess) => {
      ...
      mess.groupData = mess.isGroup ? await client.groupMetadata(mess.from): false
      if (mess.groupData) {
        mess.sender = {
          ...mess.sender,
      ...
      mess.quotedMsg = mess.quotedMsg ? (client.chats[mess.key.remoteJid] && client.chats[mess.key.remoteJid].messages[mess.quotedMsg.key.id]) || await chatMeta(mess.quotedMsg): false
      return mess;
    };
  ...
  async run() {
    try {
      ...
      global.pgDb = config.pg ? new (require('pg').Pool)(config.pg): undefined
      if (config.pg) {
        ...
        if (!query || !("object" in query)) {
          ...
          for (let a in query.object) {
            fs.writeFileSync('./src/json/'+a+'.json', JSON.stringify(query.object[a]))
          }
      ...
      global.client = new (require('./waconnection.js'))(baileys.default({
        ...
        version: (await axios.get(`https://web.whatsapp.com/check-update?version=1&platform=web`).catch(() => {
          return {
            data: {
              currentVersion: `2.2204.13`
            }}})).data.currentVersion.split('.')
      }))
      if (Object.keys(client.chats).length == 0) client.chats = database.chats ? JSON.parse(Buffer.from(database.chats, 'base64').toString()): {}
